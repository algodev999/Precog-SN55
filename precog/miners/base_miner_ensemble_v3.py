...
from calculate_score import calc_rewards
...
def forward(synapse: Challenge, cm: CMData) -> Challenge:
    ...
    prediction_interval_naive: Tuple[float, float] = get_prediction_interval_naive(
        cm=cm, timestamp=synapse.timestamp, point_estimate=point_estimate
    )
    bt.logging.debug(f"⏱️ Prediction_interval_naive: {prediction_interval_naive[0]:.3f} {prediction_interval_naive[1]:.3f}")
    interval_score_naive = calc_rewards(synapse_timestamp = synapse.timestamp, prediction_point = point_estimate, pre_interval = prediction_interval_naive)


    # Get the garch prediction interval
    prediction_interval_garch: Tuple[float, float] = get_prediction_interval_garch(
        cm=cm, timestamp=synapse.timestamp, point_estimate=point_estimate
    )
    bt.logging.debug(f"⏱️ Prediction_interval_garch: {prediction_interval_garch[0]:.3f} {prediction_interval_garch[1]:.3f}")
    
    interval_score_garch = calc_rewards(synapse_timestamp = synapse.timestamp, prediction_point = point_estimate, pre_interval = prediction_interval_garch)

    # Get the autocorrelation prediction interval
    prediction_interval_autocorrelation: Tuple[float, float] = get_prediction_interval_autocorrelation(
        cm=cm, timestamp=synapse.timestamp, point_estimate=point_estimate
    )
    interval_score_autocorrelation = calc_rewards(synapse_timestamp = synapse.timestamp, prediction_point = point_estimate, pre_interval = prediction_interval_autocorrelation)
    bt.logging.debug(f"⏱️ Prediction_interval_autocorrelation: {prediction_interval_autocorrelation[0]:.3f} {prediction_interval_autocorrelation[1]:.3f}")

    max_score = max(interval_score_naive, interval_score_garch, interval_score_autocorrelation)

    bt.logging.debug(f"💨 naive score: {interval_score_naive:.3f}, garch score: {interval_score_garch:.3f}, autocorrelation score: {interval_score_autocorrelation:.3f} ")

    if interval_score_naive == max_score:
        prediction_interval = prediction_interval_naive
    elif interval_score_garch == max_score:
        prediction_interval = prediction_interval_garch
    elif interval_score_autocorrelation == max_score:
        prediction_interval = prediction_interval_autocorrelation
    else :
        prediction_interval = prediction_interval_naive
