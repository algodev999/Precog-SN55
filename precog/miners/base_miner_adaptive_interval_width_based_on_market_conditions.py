...
def get_prediction_interval(cm: CMData, timestamp: str, point_estimate: float) -> Tuple[float, float]:
    ...
    returns = historical_price_data["ReferenceRateUSD"].pct_change().dropna()
    current_volatility = returns.rolling(window=3600).std().iloc[-1]
    long_term_volatility = returns.std()

    # Calculate VIX-like indicator (ratio of current to long-term volatility)
    volatility_ratio = current_volatility/ long_term_volatility

    # Base calculation
    sample_std_dev = float(returns.std())
    time_steps = 3600 # 60 minutes
    base_std_dev = sample_std_dev * (time_steps ** 0.5)

    # Adjust based on market conditions
    if volatility_ratio > 1.5: #High volatility regime
        adjusted_std_dev = base_std_dev * 1.5
        coefficient = 1.96 # Wider interval (95%)
    elif volatility_ratio < 0.5: # Low volatility regime
        adjusted_std_dev = base_std_dev * 0.8
        coefficient = 1.64 # Standar interval(90%)
    else: # Normal volatility
        adjusted_std_dev = base_std_dev
        coefficient = 1.64
